    ...
    >
      ...
      >
        <el-form-item label="应用名称：">
          <el-input
            v-model="forms.appName"
            placeholder="请输入应用名称"
          />
        </el-form-item>
        <el-form-item label="应用公钥：">
          <el-input
            v-model="forms.publicKey"
            placeholder="请输入应用公钥"
          />
        </el-form-item>
        <el-form-item>
    ...
    >
      ...
      <div class="setParams">
        ...
        >
          ...
          <el-form-item label="异步通知地址：">
            <el-input
              v-model="wxform.returnUrl"
              placeholder="请输入同步通知地址"
            />
          </el-form-item>
          <el-form-item label="商户ID：">
            <el-input
              v-model="wxform.mchID"
              placeholder="请输入合作者PID"
            />
          </el-form-item>
          <el-form-item label="支付秘钥：">
            <el-input
              v-model="wxform.payKey"
              placeholder="请输入支付秘钥"
            />
          </el-form-item>
          <el-form-item class="btnParent">
...
export default class extends Vue {
  ...
  private isWeixin: boolean = false
  ...
  private async getHigh() {
    // 查询b2c服务绑定状态
    let ret = await searchIsBindService(this.paramId, 'huimin_b2c')
    if (ret === 0) {
      this.isB2c = false
    } else {
      this.isB2c = true
    ...
    // 查询C2B服务绑定状态
    let rets = await searchIsBindService(this.paramId, 'huimin_c2b')
    if (rets === 0) {
      this.isC2b = false
    } else {
      this.isC2b = true
    ...
    // 查询微信支付服务是否绑定
    let retss = await searchIsBindService(this.paramId, 'wx_native')
    if (retss === 0) {
      this.isWeixin = false
    } else {
      this.isWeixin = true
    ...
    // 查询支付宝web支付服务是否绑定
    let retsss = await searchIsBindService(this.paramId, 'alipay_wap')
    if (retsss === 0) {
      this.isAlipay = false
    } else {
      this.isAlipay = true
  ...
  private async setRealParams(value: any) {
    ...
    if (value.channelCode == 'huimin_c2b') {
      this.realParam = true
    }
  }
}
